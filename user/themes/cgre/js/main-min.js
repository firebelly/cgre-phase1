(function () {
	...
	var appState = {
	  ...
	  init: function init() {
	    ...
	    document.body.appendChild(appState.breakpointIndicator); // Bind updateBreakpoints to domready and resize
	  ...
	  updateBreakpoints: function updateBreakpoints() {
	    ...
	    var breakpointIndicatorString = window.getComputedStyle(appState.breakpointIndicator, ':after').getPropertyValue('content').replace(/['"]+/g, '');
	...
	var navWaypoints = {
	  ...
	  init: function init() {
	    if ($('section.page').length) {
	      ...
	      $nav$1 = $('.nav-main');
	      $pages = $('section.page');
	      $window$1.off('scroll.navWaypoints').on('scroll.navWaypoints', navWaypoints.scrolling);
	      $window$1.off('resize.navWaypoints').on('resize.navWaypoints', navWaypoints.resizing);
	      $window$1.off('load.navWaypoints').on('load.navWaypoints', navWaypoints.resizing);
	    }
	  },
	  // Request update using requestAnimationFrame
	  requestTick: function requestTick() {
	  ...
	  update: function update() {
	    ...
	    var currentPage = ''; // Find current sticky section title based on scroll position

	    $pages.each(function (i) {
	      if (this.offsetTop - 10 <= scrollTop$1) {
	        currentPage = this.getAttribute('data-title');
	      }
	    });
	    $nav$1.find('a').removeClass('active');
	    $nav$1.find("a[data-title=\"".concat(currentPage, "\"]")).addClass('active');
	  },
	  // Scrolling
	  scrolling: function scrolling(event) {
	    scrollTop$1 = $window$1.scrollTop();
	...
	var home = {
	  init: function init() {
	    ...
	    document.querySelectorAll('.nav-main a').forEach(function (el) {
	      el.addEventListener('click', function (e) {
	        e.preventDefault(); // Ignore click if already animating
	        ...
	        if (!appState.isAnimating) {
	          ...
	          el.classList.add('active'); // Find matching page

	          var title = el.getAttribute('data-title');
	          var page = document.querySelector("section.page[data-title=\"".concat(title, "\"")); // Set state to animating for various checks

	          appState.isAnimating = true;
	          zenscroll.intoView(page, 777, function () {
	  ...
	  finalize: function finalize() {// JavaScript to be fired on the homepage, after the init JS
	  }
	};

	// Import local dependencies
	window.$ = window.jQuery = jquery; // Populate Router instance with DOM routes
	...
	appState.init(); // Init sticky nav

	stickyNav.init(); // Init nav waypoints

	navWaypoints.init(); // Load Events
